...
export function TopBar({ cameraRef }: { cameraRef: React.RefObject<CameraControllerHandle | null> }) {
  ...
  const displayTemp = tempUnit === 'F'
    ? (tempF !== null ? `${Math.round(tempF)}${unit}` : '\u2014')
    : (tempC !== null ? `${Math.round(tempC)}${unit}` : '\u2014')
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-20 flex items-center gap-3 px-4 py-3 bg-black/40 backdrop-blur-md border-b border-white/10">
      ...
      <button onClick={() => setTempUnit(tempUnit === 'C' ? 'F' : 'C')} className="text-white/60 hover:text-white text-xs border border-white/20 rounded px-2 py-1 transition-colors">
        &deg;{tempUnit === 'C' ? 'F' : 'C'}
      </button>
