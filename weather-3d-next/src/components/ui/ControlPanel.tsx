...
function Slider({ label, param, min, max, step = 0.01 }: {
  label: string; param: keyof SceneParams; min: number; max: number; step?: number
}) {
...
export function ControlPanel({ quality, onQualityChange }: {
  ...
  return (
    <div
      className={`absolute right-4 top-20 z-20 transition-all duration-300 ${collapsed ? 'w-10' : 'w-64'}`}
      style={{ maxHeight: 'calc(100vh - 6rem)' }}
    >
      {/* Collapse toggle */}
      <button
        onClick={() => setCollapsed(!collapsed)}
        className="absolute -left-4 top-3 z-30 bg-black/60 hover:bg-black/80 text-white rounded-full w-7 h-7 flex items-center justify-center text-sm border border-white/20 transition-colors shadow-lg"
      >
        {collapsed ? '\u2039' : '\u203a'}
      </button>
      ...
      {!collapsed && (
        <div
          className="bg-black/55 backdrop-blur-md rounded-2xl border border-white/10 overflow-y-auto"
          style={{ maxHeight: 'calc(100vh - 6rem)' }}
        >
          {/* Mode toggle */}
            ...
            <div className="flex bg-white/10 rounded-lg p-0.5">
              {(['real', 'manual'] as const).map(m => (
                <button key={m} onClick={() => setMode(m)}
                  className={`flex-1 text-xs py-1.5 rounded-md transition-all ${mode === m ? 'bg-white text-black font-semibold' : 'text-white/60 hover:text-white'}`}>
            ...
            <div className="flex gap-1">
              {(['low', 'medium', 'high'] as const).map(q => (
                <button key={q} onClick={() => onQualityChange(q)}
                  className={`flex-1 text-xs py-1 rounded transition-all ${quality === q ? 'bg-white/20 text-white' : 'text-white/40 hover:text-white/70'}`}>
            ...
            <>
              <div className="flex border-b border-white/10">
                {(['sky', 'clouds', 'effects'] as const).map(s => (
                  <button key={s} onClick={() => setSection(s)}
                    className={`flex-1 text-xs py-2 capitalize transition-colors ${section === s ? 'text-white border-b-2 border-white' : 'text-white/40 hover:text-white/60'}`}>
                    {s}
                  </button>
                ))}
              </div>
              <div className="p-3">
                {section === 'sky' && (<>
                  ...
                  <Slider label="Turbidity"      param="turbidity"        min={1}   max={16}   step={0.5} />
                  <Slider label="Rayleigh"       param="rayleigh"         min={0}   max={3} />
                  <Slider label="Fog Density"    param="fogDensity"       min={0}   max={0.003} step={0.00005} />
                </>)}
                {section === 'clouds' && (<>
                  ...
                  <div className="mb-2">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {(['cumulus','stratus','cirrus','storm'] as const).map(t => (
                        <button key={t} onClick={() => setSceneParam('cloudType', t)}
                          className={`text-xs px-2 py-1 rounded transition-all ${sceneParams.cloudType === t ? 'bg-white/30 text-white' : 'text-white/40 hover:text-white/70 bg-white/5'}`}>
                          {t}
                        </button>
                      ))}
                    </div>
                ...
                {section === 'effects' && (<>
                  <Slider label="Bloom"      param="bloomStrength"     min={0}   max={2} />
          ...
          {mode === 'real' && (
            <div className="p-3">
              ...
              {[
                ...
                { label: 'Fog Density',    value: sceneParams.fogDensity.toFixed(5) },
      ...
      {collapsed && (
        <div className="bg-black/55 backdrop-blur-md rounded-2xl border border-white/10 p-2 flex flex-col items-center gap-2">
