...
export const WeatherScene = forwardRef<CameraControllerHandle, { googleSession: string | null; quality: 'low' | 'medium' | 'high' }>(
  function WeatherScene({ googleSession, quality }, cameraRef) {
    const { location, sceneParams } = useAppStore()
    return (
      <Canvas
        shadows={quality !== 'low'}
        gl={{ antialias: quality !== 'low', toneMapping: 4, toneMappingExposure: 1.0, powerPreference: 'high-performance' }}
      ...
      >
        ...
        <PostProcessing params={sceneParams} quality={quality} />
        <Preload all />
      </Canvas>
    )
  }
